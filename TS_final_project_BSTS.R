...
"
Based on the auto.arima for each of the three transformations, an AR(5) for both the differenced and seasonally differnces transformations
...
pred_diff <- predict(model_diff, horizon = 59)
...
ss_diff_ar <- AddAr(order = 5, y = diff_train, state.specification = ss_diff_ar)
...
# forecast the next 59 days (Jan 2025 - Feb 2025)
pred_diff_ar <- predict(model_diff_ar, horizon = 59)
...
ss_bc_ar <- AddAr(order = 2, y = diff_boxcox_train, state.specification = ss_bc_ar)
