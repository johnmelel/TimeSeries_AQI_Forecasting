...
These models assume stationarity, independence of residuals, and a normally distributed error structure. The stationarity assumption was tested using the Augmented Dickey-Fuller (ADF) test, which showed significant p-values, indicating stationarity after differencing. The KPSS test confirmed this stationarity, as all p-values were greater than 0.1. Additionally, the Ljung-Box test was used to check for autocorrelation in the residuals, and the results suggested that the ARFIMA model had the least amount of autocorrelation, further supporting its suitability. Normality of residuals was examined using histograms and Q-Q plots, which suggested that ARFIMA residuals were closest to a normal distribution compared to ARIMA and SARIMA.
...
```{r}
knitr::include_graphics("ACF_ARFIMA.png")
```

```{r}
knitr::include_graphics("ACF_SARIMA.png")
```
...
```{r}
knitr::include_graphics("Residuals_FItted_ARFIMA.png")
```

```{r}
knitr::include_graphics("Residuals_FItted_SARIMA.png")
```
...
The models' performances were evaluated using Root Mean Squared Error (RMSE):
...
The forecast plot shows projected values for the next period with confidence intervals. The forecast aligns well with historical patterns, indicating that ARFIMA effectively captures the underlying time series structure. The confidence intervals remain relatively narrow, signifying a stable prediction range. The mean absolute percentage error (MAPE) was also computed, confirming that ARFIMA maintained the lowest error rate across all forecasted values, reinforcing its suitability for this dataset.
