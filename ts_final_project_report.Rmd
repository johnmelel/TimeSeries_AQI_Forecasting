...
The Residuals vs. Fitted plots reveal that errors are randomly distributed without any significant pattern, confirming that the models appropriately capture the variance in the data. However, upon closer examination,  we see clear signs of heteroscedasticity, suggesting that these models do not adequately capture variance structure in the data. Thus, model assumptions regarding homoscedasticity are violated, further questioning the reliability of these models.
...
The ETS/Holt Winters models basically smoothens the historic data to make a prediction, so the trade-off is that it is not necessary good at  predicting sudden changes (sudden AQI change during wildfires etc), and it tends to overfit for our data.
...
The BSTS model shows promising results, but the trade-off is that the model is computationally heavy in order to properly capture the complexity. 
