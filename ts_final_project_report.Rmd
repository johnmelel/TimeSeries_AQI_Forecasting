...
Air pollution is a critical public health concern in urban areas, particularly in London, where long-term exposure to poor air quality has historically contributed to respiratory diseases, cardiovascular issues, and reduced life expectancy. London has faced severe air pollution problems for centuries, from the coal-driven smogs of the 17th century to the infamous Great Smog of 1952, which led to the Clean Air Acts of 1956 and 1968. However, air pollution remained a persistent issue into the late 20th and early 21st centuries. In the 1990s, studies highlighted the dangers of nitrogen dioxide (NO₂) and particulate matter from vehicle emissions, particularly from the rise of diesel cars. By the early 2000s, London was struggling to meet European Union air quality standards, leading to policies such as the introduction of the Congestion Charge in 2003 to reduce traffic pollution. Despite these efforts, air pollution continues to pose a significant challenge due to vehicle emissions, industrial activity, and urban density. The importance of monitoring and forecasting air quality in London cannot be overstated, as it directly affects public health, the economy, and the overall livability of the city. Timely intervention, such as issuing health advisories, implementing traffic control measures, and enforcing emissions regulations, can help mitigate these risks. By forecasting the Air Quality Index (AQI), stakeholders like the Greater London Authority, UK Environment Agency, Public Health England, Transport for London, and industrial sectors can make informed decisions on pollution control, public health strategies, and urban planning. Additionally, London residents, commuters, and environmental advocacy groups all stand to benefit from better air quality management.
...
These models assume stationarity, independence of residuals, and a normally distributed error structure. To assess the model assumptions, ACF plots and Residuals vs. Fitted plots were examined:
...
The ACF plots for the ARFIMA model indicate  residual autocorrelation. However, the SARIMA model, in particular, exhibits significant autocorrelation, especially in seasonal components. This suggests that the SARIMA model has unaccounted-for structure in its residuals. Due to this, the residual assumptions of the model are violated.

```{r}
knitr::include_graphics("ARIMA_HJ/Plots_final/residuals_arfima.png")
```

```{r}
knitr::include_graphics("ARIMA_HJ/Plots_final/residuals_sarima.png")
```

The Residuals vs. Fitted plots reveal that errors are not randomly distributed without any significant pattern. We see clear signs of heteroscedasticity, suggesting that these models do not adequately capture the variance structure in the data. Thus, model assumptions regarding homoscedasticity are violated.
...
The forecast plot shows projected values for the next period with confidence intervals. The forecast plot shows projected values for the next period with confidence intervals. The forecast aligns well with historical patterns, indicating that ARFIMA could potentially capture the underlying time series structure. The confidence intervals remain relatively narrow, signifying a stable prediction range.

Despite ARFIMA’s relatively better performance, the overall analysis indicates that key model assumptions are violated for all models. The ACF plots highlight significant autocorrelation in all of the models, especially with the SARIMA residuals, contradicting the assumption of white noise errors. The residual vs. fitted plots suggest heteroscedasticity, further undermining the validity of these models. Given these violations, it is evident that none of these models provide a perfect fit, and further refinements—such as incorporating alternative differencing methods or hybrid modeling approaches—are needed to improve predictive accuracy.

The ARFIMA, ARIMA, SARIMA models simpler models which are easy to interpret, however the trade-off is at the cost of not capturing all underlying time series patterns.
...
The ETS and Holt-Winters models essentially smooth the historic data to make a prediction. Due to this there is a trade-off with the prediction ability of the ETS model when it comes to sudden changes in AQI. The ETS model is better at predicting gradual changes compared to sudden changes. For example, it would not effectively predict AQI changes in the event of natural disasters such as wildfires. Additionally, the trade-off for Holt-Winters model is that it tends to overfit to the train data.
...
rmse_data <- data.frame(
  ...
  RMSE = c(16.85, 22.64, 43.62, 43.94, 31.61, 22.64)
)
...
For our final model, we used LSTM on as it outperforms traditional models for complex, nonlinear time series data, especially when long-term dependencies exist. We created a stacked model with double layer LSTM for learning the AQI data and two dense layers for predicting output, and our sequence window was for 60 days. The data was also scaled using Min-Max Scaling for consistency. The model was trained for 5 epochs and, overall, gave great predictions with relatively low RMSE. Since the model is capable of capturing trends and long-term dependencies in the data, we felt that it was sufficient to only construct this model on the actual data.
...
Anusha - EDA, exponential smoothing models, coalescing final code R script, formatting final report RMD, and coalescing individual member write-ups, presented.

Sara - EDA, BSTS models, data transformations, discussion, presented.

Hritik - EDA, ARIMA/SARIMA/ARFIMA models, intro.

John - EDA, LSTM models, made github repository, github readme, formatting final report RMD, presented.
